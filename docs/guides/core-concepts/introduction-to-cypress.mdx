...
After you're done, we suggest watching some of our <Icon
  name="video"
  inline="true"
  url="/examples/tutorials"
  callout="Tutorial Videos"
/>.
...
cy
  ...
  .then(($myElement) => {
    // ...message the subject with some arbitrary code
...
[write your own assertions as Chai plugins](/examples/recipes#Fundamentals) and
use them in Cypress.
...
Check out our example recipes for [unit testing](/examples/recipes) and
[unit testing React components](/examples/recipes#Unit-Testing).
